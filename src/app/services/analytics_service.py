...
def get_headcount_trend(
    ...
    return (
        query.order_by(HeadcountTrend.year.desc(), HeadcountTrend.month.desc()).limit(limit).all()
    )
...
def get_department_metrics(
    ...
    return (
        query.order_by(DepartmentMonthlyMetrics.year.desc(), DepartmentMonthlyMetrics.month.desc())
        .limit(limit)
        .all()
    )
...
def get_employee_attendance(
    ...
    return (
        query.order_by(
            EmployeeAttendanceMetrics.year.desc(), EmployeeAttendanceMetrics.month.desc()
        )
        .limit(limit)
        .all()
    )
...
def get_organization_metrics(
    ...
    return (
        query.order_by(OrganizationMetrics.year.desc(), OrganizationMetrics.month.desc())
        .limit(limit)
        .all()
    )
...
def get_timesheet_daily_summary(
    ...
    if employee_id:
        query = query.filter(TimesheetDailySummary.client_employee_id == employee_id)
    if start_date:
        query = query.filter(TimesheetDailySummary.work_date >= start_date)
    if end_date:
        query = query.filter(TimesheetDailySummary.work_date <= end_date)

    return query.order_by(TimesheetDailySummary.work_date.desc()).offset(skip).limit(limit).all()
