...
class BronzeLoader:
    ...
    def load_employee_data(
        ...
        try:
                ...
                for _, row in batch.iterrows():
                    bronze_record = EmployeeBronze(
                        ...
                        manager_employee_id=str(
                            row.get("manager_employee_id", "")),
                        manager_employee_name=str(
                            row.get("manager_employee_name", "")),
                        ...
                        scheduled_weekly_hour=str(
                            row.get("scheduled_weekly_hour", "")),
            ...
            logger.info(
                f"Successfully loaded {records_loaded} employee records from {source_file}")
    ...
    def load_timesheet_data(
        ...
        try:
                ...
                for _, row in batch.iterrows():
                    ...
                    bronze_record = TimesheetBronze(
                        ...
                        home_department_id=str(
                            row.get("home_department_id", "")),
                        home_department_name=str(
                            row.get("home_department_name", "")),
                        pay_code=str(row.get("pay_code", "")),
                        punch_in_comment=str(row.get("punch_in_comment", "")),
                        punch_out_comment=str(
                            row.get("punch_out_comment", "")),
                        hours_worked=hours_worked,
                        punch_apply_date=str(row.get("punch_apply_date", "")),
                        punch_in_datetime=str(
                            row.get("punch_in_datetime", "")),
                        punch_out_datetime=str(
                            row.get("punch_out_datetime", "")),
                        scheduled_start_datetime=str(
                            row.get("scheduled_start_datetime", "")),
                        scheduled_end_datetime=str(
                            row.get("scheduled_end_datetime", "")),
                        source_file=source_file,
                        created_at=datetime.utcnow(),
                    )
                    bronze_records.append(bronze_record)
            ...
            self.db_session.commit()
            logger.info(
                f"Successfully loaded {records_loaded} timesheet records from {source_file}")
            return records_loaded
